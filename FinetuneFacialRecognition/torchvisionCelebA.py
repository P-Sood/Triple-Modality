...
def main(param_dict):
    ...
    train_dataset = datasets.CelebA(
        data_dir, split="train", transform=train_transform, target_type="identity"
    )
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    valid_dataset = datasets.CelebA(
        data_dir, split="valid", transform=data_transform, target_type="identity"
    )
    valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size)

    test_dataset = datasets.CelebA(
        data_dir, split="test", transform=data_transform, target_type="identity"
    )
    ...
    for epoch in tqdm(range(epochs)):
        ...
        train_loss, train_acc = train(
            model, epoch, train_dataloader, criterion, optimizer, scheduler, device
        )
        wandb.log(
            {
...
def parser():
    parser = argparse.ArgumentParser(
        description="Fine-tune InceptionResnetV1 on CelebA dataset"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        help="path to CelebA dataset",
        default="/home/jupyter/multi-modal-emotion/FinetuneData/celeba",
    )
    parser.add_argument(
        "--epoch", type=int, help="number of epochs to train for", default=10
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--weight_decay", type=float, help="weight decay for optimizer", default=0.0001
    )
    parser.add_argument(
        "--T_max", type=int, help="T_max for cosine annealing scheduler", default=3
    )
    parser.add_argument(
