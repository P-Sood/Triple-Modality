...
def get_times(path , model_cuda , text , labels , sample_rate , enable_gpu):
    ...
    with torch.inference_mode():
        ...
        waveform = SEP.separate(waveform.T) # This is where we separate the vocals from the audio, using spleeter
        ...
        if enable_gpu and waveform.shape[1] < 3000000 and text is not "[N/A]":
            ...
            for enum , part in enumerate(parts):
                input_values = PROC(part, sampling_rate=sample_rate, return_tensors="pt").input_values
                ...
                timings[enum] = general_backtrack(transc , size , sample_rate , emission , labels)
                del emission
...
def arg_parse():
    ...
    parser.add_argument("--dataset"  , "-d", help="The dataset we are using currently", default = "../data/tiktok.pkl" , type=str) 
    parser.add_argument("--enable_gpu" , "-e" , help="Use a GPU or not"  , default=True, type=bool)
    return parser.parse_args()


def fun(df , dataset , i): 
    sub_df = df.iloc[i*40:(i+1)*40]
    sub_df['timings'] = sub_df.progress_apply(lambda x: get_times(x['audio_path'] , model_cuda , x['text'].split(":")[-1][1:] , labels , sr , enable_gpu) , axis = 1)
    sub_df.to_pickle(f"{dataset[:-4]}_{i}_new.pkl") # change this to the next applicable one in case it crashes
...
if __name__ == "__main__":
    ...
    tqdm.pandas()
    for i in tqdm(range(0,25)): # change first arg in range, to the next applicable one in case it crashes
        fun(df , dataset, i)    
