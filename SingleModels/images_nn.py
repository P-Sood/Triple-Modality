...
def runModel(rank, df_train, df_val, df_test, param_dict, model_param, wandb):
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=weights.to(rank)).to(
        rank
    )  # TODO: maybe error here?
    Metric = Metrics(num_classes=num_labels, id2label=id2label, rank=rank)
    df_train = prepare_dataloader(
        df_train, batch_size=batch_size
    )  # TODO: maybe prepare dataloader?
...
def main():
    ...
    try:
        weights = torch.Tensor(
            list(
                dict(
                    sorted((dict(1 - (df["emotion"].value_counts() / len(df))).items()))
                ).values()
            )
        )
        ...
        df_train, df_val, _, __ = train_test_split(
            df_train,
            df_train["emotion"],
            test_size=0.25,
            random_state=param_dict["seed"],
        ...
        weights = torch.Tensor(
            list(
                dict(
                    sorted((dict(1 - (df["label"].value_counts() / len(df))).items()))
                ).values()
            )
        )
        # weights = torch.Tensor([1,1,1,1,1,1,1])
    ...
    param_dict = {
        ...
        "T_max": 10,
        "seed": 96,
        "weights": torch.Tensor([0.3667, 0.6333]),
...
if __name__ == "__main__":
    ...
    tqdm.pandas()
    df["timings"] = df.progress_apply(
        lambda x: get_times(x.audio_path, x.text, model, labels, sr), axis=1
    )
