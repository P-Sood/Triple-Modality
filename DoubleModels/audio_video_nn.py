...
def prepare_dataloader(df , dataset , batch_size, label_task , epoch_switch , pin_memory=True, num_workers=2 , check = "train" , accum = False ): 
    ...
    if accum:
        ...
        dataset = AudioVideoDataset(df , dataset , batch_size, feature_col1="audio_path" , feature_col2="video_path" , label_col=label_task , timings="timings" , accum=accum , check=check)
    else:
        dataset = AudioVideoDataset(df , dataset , batch_size, feature_col1="audio_path" , feature_col2="video_path" , label_col=label_task , timings="timings" , accum=accum , check=check)
