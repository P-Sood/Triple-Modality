...
def get_statistics(
    ...
    Metric,
    check="train",
    location="/home/prsood/projects/def-whkchun/prsood/multi-modal-emotion/Inference/visbertTest.txt",
):
    batch_loss = None
    # print(" \n inside get stats \n")
    ...
    output = our_model(
        ...
        visual_attention_mask=img_attention_mask,
        visual_token_type_ids=img_token_type_ids,
        check=check,
    )
...
def one_epoch(train_dataloader, model, criterion, optimizer, clip, Metric):
    ...
    for train_input, train_label in tqdm(train_dataloader, desc="training"):
        train_batch_loss, total_loss_train = get_statistics(
            train_input, train_label, model, criterion, total_loss_train, Metric
        )
    ...
    return (
        ...
        optimizer,
        train_batch_loss,
        total_loss_train / len(train_dataloader.dataset),
    )
...
def validate(val_dataloader, model, criterion, Metric, name="val", location="val"):
    ...
    with torch.no_grad():
        for val_input, val_label in tqdm(
            val_dataloader, desc="validate" if name == "val" else "testing"
        ):
            val_batch_loss, total_loss_val = get_statistics(
                val_input,
                val_label,
                model,
                criterion,
...
def train_vbert_network(
    ...
    for epoch_num in tqdm(range(epochs), desc="epochs"):
        ...
        model, optimizer, train_batch_loss, train_loss = one_epoch(
            train_dataloader, model, criterion, optimizer, clip, Metric
        )
        (
            multiAcc,
            multiF1,
        ...
        ):  # this is to make sure that gradietn descent isnt stuck in a local minima
            if earlystop(model, val_loss):
                model = earlystop.best_state
    torch.save(
        model.state_dict(),
        "/home/prsood/projects/def-whkchun/prsood/multi-modal-emotion/Inference/VisBert.pt",
    )
    return model
