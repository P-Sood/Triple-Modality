...
class TextAudioVideoDataset(Dataset):
    ...
    def __getitem__(self, idx): 
        return [self.texts[idx] , self.Data.speech_file_to_array_fn(self.audio_path[idx] , self.timings[idx] , self.check ) , 
                self.Data.videoMAE_features(self.video_path[idx] , self.timings[idx] , self.check)] , np.array(self.labels[idx])
...
class AudioVideoDataset(Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        return [self.Data.speech_file_to_array_fn(self.audio_path[idx] , self.timings[idx] , self.check ), self.Data.videoMAE_features(self.video_path[idx] , self.timings[idx] , self.check)] , np.array(self.labels[idx])
...
class TextAudioDataset(Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        return [self.texts[idx] , self.Data.speech_file_to_array_fn(self.audio_path[idx] , self.timings[idx] , self.check )] , np.array(self.labels[idx])
...
class TextVideoDataset(Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        return [self.texts[idx] , self.Data.videoMAE_features(self.video_path[idx] , self.timings[idx] , self.check)] , np.array(self.labels[idx])
...
class VisualDataset(Dataset):
    ...
    """
    def __init__(self, df , dataset , batch_size , feature_col , label_col ,  timings = None  , accum = False , check = "test"):
    ...
    def __getitem__(self, idx): return  self.Data.videoMAE_features(self.video_path[idx] , self.timings[idx] , self.check) , np.array(self.labels[idx])
...
class Wav2VecAudioDataset(Dataset):
    def __init__(self, df , dataset , batch_size , feature_col , label_col , accum = False , check = "test"):
        """
    ...
    def __getitem__(self, idx: int): return self.Data.speech_file_to_array_fn(self.audio_path[idx] , self.timings[idx] , self.check ), np.array(self.labels[idx])
...
class Data:
    ...
    def __init__(self , video , audio) -> None:
        self.VIDEOS =  h5py.File(video,'r', libver='latest' , swmr=True) if video is not None else None
        self.AUDIOS =  h5py.File(audio,'r', libver='latest' , swmr=True) if audio is not None else None
        self.must = False
    ...
    def videoMAE_features(self , path , timings, check):
        ...
        if not self.must:
            video = torch.Tensor(self.VIDEOS[f"{check}_{path.split('/')[-1][:-4]}_{timings}"][()]  ) # H5PY, how to remove data after loading it into memory
