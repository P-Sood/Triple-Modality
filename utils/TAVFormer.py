...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim=768, n_heads=12):
        ...
        )  # 768/12 = 64  . each key,query, value will be of 768d

        # key,query and value matrixes    #64 x 64
        self.query_matrix = nn.Linear(
            self.embed_dim, self.embed_dim, bias=False
        )  # single key matrix for all 12 keys #768x768
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        return (
            tensor.view(bsz, seq_len, self.n_heads, self.single_head_dim)
            .transpose(1, 2)
            .contiguous()
        )

    def forward(
        self, hidden_states, attention_mask=None, early_div=False
    ):  # batch_size x sequence_length x embedding_dim    # 4 x 858 x 768
        """
        Args:
        ...
        if attention_mask is not None:
            ...
            )  # (batch_size , 1 , 1 , seq_length) ->(batch_size , 1 , seq_length , seq_length)
            attn_weights = (
                attn_weights.view(bsz, self.n_heads, tgt_len, src_len) + attention_mask
            )
        ...
        scores = torch.matmul(
            scores, value_states
        )  ##(4x12x 858x 858) x (4 x 12 x 858 x 64) = (4 x 12 x 858 x 64)
        ...
        concat = (
            ...
            .contiguous()
            .view(bsz, src_len, self.single_head_dim * self.n_heads)
        )  # (4x12x858x64) -> (4x858x12x64)  -> (4,858,768)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, early_div=False):
        ...
        attention_residual_out = hidden_states + self.attention(
            hidden_states, attention_mask, early_div
        )  # 32x10x512
        del attention_mask
...
class VideoMAESelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask.expand(
                -1, 1, attention_mask.shape[-1], -1
            )  # (batch_size , 1 , 1 , seq_length) ->(batch_size , 12 , seq_length , seq_length)
