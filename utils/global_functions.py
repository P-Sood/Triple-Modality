...
class Metrics:
    ...
    def __init__(
        ...
        self.confusionMatrix = MulticlassConfusionMatrix(
        ...
        self.multiRec = MulticlassRecall(
            self.num_classes,
            self.top_k,
            self.average,
            self.multidim_average,
        ...
        self.multiPrec = MulticlassPrecision(
            self.num_classes,
            self.top_k,
            self.average,
            self.multidim_average,
        ...
        self.multiAcc = MulticlassAccuracy(
            self.num_classes,
            self.top_k,
            self.average,
            self.multidim_average,
        ...
        self.scalarMacroF1 = MulticlassF1Score(
            self.num_classes,
            self.top_k,
            "macro",
            self.multidim_average,
        ...
        self.scalarWeightedF1 = MulticlassF1Score(
            self.num_classes,
            self.top_k,
            "weighted",
            self.multidim_average,
        ...
        self.scalarPrec = MulticlassPrecision(
            self.num_classes,
            self.top_k,
            "macro",
            self.multidim_average,
        ...
        self.scalarRec = MulticlassRecall(
            self.num_classes,
            self.top_k,
            "macro",
            self.multidim_average,
        ...
        self.scalarAcc = MulticlassAccuracy(
            self.num_classes,
            self.top_k,
            "weighted",
            self.multidim_average,
    ...
    def compute_scores(self, name):  # name is train test or val
        ...
        return (
            {
                name + "/" + "multiAcc/" + self.id2label[v]: k.item()
                for v, k in enumerate(multiAccuracy)
            },
            {
                name + "/" + "multiF1/" + self.id2label[v]: k.item()
                for v, k in enumerate(multiF1)
            },
            {
                name + "/" + "multiRec/" + self.id2label[v]: k.item()
                for v, k in enumerate(multiRec)
            },
            {
                name + "/" + "multiPrec/" + self.id2label[v]: k.item()
                for v, k in enumerate(multiPrec)
            },
            scalarAccuracy,
            scalarMacroF1,
            scalarWeightedF1,
...
def save_model(model, optimizer, criterion, scheduler, epoch, step, path, log_val):
    ...
    if epoch == 0 and step < log_val + 1:
        ...
        try:
            os.makedirs(
                f"{path}/{wandb.run.project}/{wandb.run.sweep_id}/{wandb.run.name}/"
            )
        except:
            os.makedirs(
                f"{path}/{wandb.run.project}/{wandb.run.sweep_id}/copy_{wandb.run.name}/"
            )
    try:
        torch.save(
            {
...
def load_model(model, optimizer, criterion, path):
    ...
    print(
        f"Current best model is on epoch {checkpoint['epoch']}, and step {checkpoint['step']} on path {p}",
        flush=True,
    )
...
def arg_parse(description):
    ...
    parser.add_argument(
        "--clip", "-c", help="Set the gradient clip", default=1.0, type=float
    )
    parser.add_argument(
        "--epoch_switch",
        "-es",
    ...
    parser.add_argument(
        "--dropout",
        "-dr",
        help="the rate for each dropout layer",
        default=0.5,
        type=float,
    )
    parser.add_argument(
        "--num_layers",
        "-nl",
        help="the number of layers in our transformers model",
        default=12,
        type=int,
    )
    parser.add_argument(
