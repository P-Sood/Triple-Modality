...
class TAVEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.f = h5py.File(
            f"../../data/{dataset_name}.features.hdf5", "a", liver="latest", swmr=True
        )
    ...
    def forward(
        ...
        if self.must:
            self.f.create_dataset(
                f"{check}/{video_path[0][0].split('/')[-1][:-4]}_{timings[0]}/text",
                data=text_outputs.cpu().detach().numpy(),
            )
        else:
            self.f.create_dataset(
                f"{check}/{video_path[0].split('/')[-1][:-4]}_{timings[0]}/text",
                data=text_outputs.cpu().detach().numpy(),
            )
        ...
        if self.must:
            ...
            self.f.create_dataset(
                f"{check}/{video_path[0][1].split('/')[-1][:-4]}_{timings[0][1]}/audio_context",
                data=aud_context.cpu().detach().numpy(),
            )
            self.f.create_dataset(
                f"{check}/{video_path[0][0].split('/')[-1][:-4]}_{timings[0][0]}/audio",
                data=aud_outputs.cpu().detach().numpy(),
            )
            del aud_context
        else:
            self.f.create_dataset(
                f"{check}/{video_path[0].split('/')[-1][:-4]}_{timings[0]}/audio",
                data=aud_outputs.cpu().detach().numpy(),
            )
        ...
        if self.must:
            ...
            del video_context
            self.f.create_dataset(
                f"{check}/{video_path[0][1].split('/')[-1][:-4]}_{timings[0][1]}/video_context",
                data=vid_context.cpu().detach().numpy(),
            )
            self.f.create_dataset(
                f"{check}/{video_path[0][0].split('/')[-1][:-4]}_{timings[0][0]}/video",
                data=vid_outputs.cpu().detach().numpy(),
            )
        else:
            self.f.create_dataset(
                f"{check}/{video_path[0].split('/')[-1][:-4]}_{timings[0]}/video",
                data=vid_outputs.cpu().detach().numpy(),
            )
